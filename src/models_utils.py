...
from .models import Lexeme, Grammar, EPSILON

def build_lukera_lexeme() -> Lexeme:
    """
    Constrói e retorna as regras léxicas específicas da linguagem Lukera.
    """
    ...
    return Lexeme(keywords=keywords, operators=operators, delimiters=delimiters)
        ...
        for prod in prods:
            ...
            for terminal in first_alpha - {EPSILON}:
                if terminal in table[A]:
                    print(f"[CRITICAL] Conflict LL(1) in M[{A}, {terminal}]. Rules colliding!")
            ...
            if EPSILON in first_alpha:
                for terminal in follow[A]:
                    if terminal in table[A]:
                        print(f"[CRITICAL] Conflict LL(1) (Follow) in M[{A}, {terminal}]")
                    table[A][terminal] = prod
                    
    return table


def parsing_table_pandas(grammar:Grammar, first:dict, follow: dict) -> pd.DataFrame:
    """
