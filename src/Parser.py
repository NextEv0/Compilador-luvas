...
from .Grammar import Grammar, compute_first, compute_follow, build_parsing_table, EPSILON
...
class LukeraParser:
    ...
    def parse(self):
        ...
        print(f" Iniciando análise de {len(self.tokens)} tokens...")

        while len(stack) > 0:
            step += 1
            if step > max_steps:
                print(" [ERRO FATAL] Loop infinito detectado no parser.")
                break
            ...
            # CASO 2: Topo é Terminal (mas diferente do token) -> ERRO
                ...
                action = f"ERRO: Esperava '{top}', veio '{token_val}'"
                self._log_trace(matched_str, stack, input_view, action)
            ...
            # CASO 3: Topo é Não-Terminal
                ...
                if production is not None:
                    ...
                    if token_type in follow_set or "EOF" in follow_set:
                        ...
                        action = f"ERRO (Pânico): Pop {top} (Sincroniza via Follow)"
                        self._log_trace(matched_str, stack, input_view, action)
                        stack.pop()
                    else:
                        # Sincronização: Descarta Token (Pula entrada)
                        action = f"ERRO (Pânico): Descarta '{token_val}'"
                        self._log_trace(matched_str, stack, input_view, action)
                        cursor += 1

    def _log_trace(self, matched, stack, inp, action):
        """Salva o estado atual para a tabela visual."""
        # Inverte a lista da pilha visualmente para ficar igual à teoria (Topo à esquerda ou direita)
        # Na imagem que você mandou, o topo está à direita.
